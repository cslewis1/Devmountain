...
const map3 = map1.map(element => String(element))
console.log(map3)
...
let map5 = map4.map(el => el.name = `President ${el.name}`)
console.log(map5)
...
let oddArray = arrNum.filter(el => el % 2 === 1)
...
let newArr = arrNum.filter((el, ind) => el % 2 === 1 && ind % 2 === 1)
console.log(newArr)
...
let lowerArray = arrString.filter(el => el.includes('a'))
...
let newSum = 0;
theNumArr.forEach(number => newSum += number)
console.log(newSum)
