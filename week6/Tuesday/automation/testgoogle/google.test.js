...
afterAll(async () => {
  ...
  test("can search Google twice", async () => {
    ...
    await driver.findElement(By.name("q")).sendKeys("gardens", Key.RETURN);
    await driver.wait(until.titleIs("gardens - Google Search"), 1000);
    ...
    .sendKeys("pilates", Key.RETURN);
    // TODO Wait for the results page to load
    await driver.wait(until.titleIs("pilates - Google Search"), 1000);
  });
